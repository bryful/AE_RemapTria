       ...
       var ret = false;
		...
		if (aeremapCall.exists==true){
			try{
                ...
                r = system.callSystem(cmd + " /center");
...
//btnApply:Button{alignment:["fill","top"],text:"適応"},\
    ...
    */
    ...
    var makeRbtn = function(ary)
    {
        /*
        Panel{
            alignment:["fill","fill"],orientation:"row",text:"Cells",
            rb00:RadioButton{alignment:["fill","top"],text:"name"}
            }
        */
        clearRbtns();
        if (ary.length>0)
        {
            winObj.gr.g1.pnlCells.orientation = "column";
            winObj.gr.g1.pnlCells.alignment = ["fill","fill"];
            for (var i=0; i<ary.length;i++)
            {
                res+=''
                var p = winObj.gr.g1.pnlCells.add(
                    "button",
                    undefined,
                    ary[i] +" :Apply",
                    {alignment:["fill","top"],justify:"right"});
